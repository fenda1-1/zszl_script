...
public class RenderWorldLastEvent extends Event {
    ...
    public static final class WorldRenderContext {
        ...
        private static WorldRenderContext create(Camera camera) {
            ...
            Matrix3f viewRotation = new Matrix3f(inverseViewRotation);
            if (Math.abs(viewRotation.determinant()) < 1.0E-6F) {
                return null;
            }
            viewRotation.invert();
            return new WorldRenderContext(camera, camera.getPosition(), matrixFromRotation(viewRotation));
        }
        ...
        public PoseStack createWorldPoseStack() {
            ...
            poseStack.mulPoseMatrix(new Matrix4f(viewRotationMatrix));
            return poseStack;
        }
