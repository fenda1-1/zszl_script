...
public final class ItemFilterHandler {
    ...
    public static void dropItemsByNameFilter() {
        ...
        for (int inventoryIndex = 0; inventoryIndex < player.getInventory().getNonEquipmentItems().size(); inventoryIndex++) {
            ItemStack stack = player.getInventory().getNonEquipmentItems().get(inventoryIndex);
    ...
    private static boolean canPlaceIntoTarget(ItemStack carried, Slot targetSlot) {
        ...
        if (!ItemStack.isSameItemSameComponents(targetStack, carried)) {
            return false;
        }
    ...
    private static List<Integer> buildInventoryMenuSlotList(AbstractContainerMenu menu, LocalPlayer player,
        ...
        List<Integer> result = new ArrayList<>();
        int inventorySize = player == null ? 0 : player.getInventory().getNonEquipmentItems().size();
