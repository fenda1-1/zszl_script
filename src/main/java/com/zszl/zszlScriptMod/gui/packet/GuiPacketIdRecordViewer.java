    ...
    private void copySelectedRecords() {
        ...
        if (mc.player != null) {
            mc.player.displayClientMessage(new TextComponentString(
                    ChatFormatting.GREEN + "已复制 " + selectedIndices.size() + " 条数据包ID记录"), false);
        }
    }
        ...
        switch (button.id) {
            ...
            case 2:
                if (PacketIdRecordManager.clearRecords()) {
                    if (mc.player != null) {
                        mc.player.displayClientMessage(new TextComponentString(ChatFormatting.GREEN + "已清空数据包ID记录"), false);
                    }
                    selectedIndices.clear();
                    lastSelectedIndex = -1;
                    reloadRecords();
