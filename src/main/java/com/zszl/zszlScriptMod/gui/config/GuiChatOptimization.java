...
public class GuiChatOptimization extends ThemedGuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (graphics != null) {
            ...
            graphics.pose().translate(this.panelX, this.contentViewportTop - this.scrollOffset, 0.0F);
    ...
    private void drawExampleChat() {
        ...
        int wrapWidth = Mth.floor((float) this.mc.gui.getChat().getWidth() / Math.max(chatScale, 0.001F));
        ...
        int chatWidth = this.mc.gui.getChat().getWidth();

        graphics.pose().pushPose();
        graphics.pose().translate(4.0F + this.settings.xOffset, baseY + this.settings.yOffset, 0.0F);
    ...
    private void drawPreviewBackground(GuiGraphics graphics, int x, int y, int width, int height, int alpha) {
        ...
        var background = TextureManagerHelper.getResourceLocationForPath(this.settings.backgroundImagePath,
        ...
        if (background != null) {
            ...
            RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, (alpha / 255.0F) * opacityRatio);
