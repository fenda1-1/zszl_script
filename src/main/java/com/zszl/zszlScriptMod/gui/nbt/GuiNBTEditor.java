...
public class GuiNBTEditor extends ThemedGuiScreen {
    ...
    public GuiNBTEditor(GuiScreen parent, ItemStack stack) {
        ...
        this.nbt = this.itemStack.hasTag() && this.itemStack.getTag() != null
                ? this.itemStack.getTag().copy()
                : new CompoundTag();
        parseNbtToList();
    }
    ...
    private void rebuildNbtFromList() {
        for (String key : new ArrayList<>(this.nbt.getAllKeys())) {
            this.nbt.remove(key);
        }
