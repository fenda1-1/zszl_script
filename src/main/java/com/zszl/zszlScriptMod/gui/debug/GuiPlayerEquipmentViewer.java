...
public class GuiPlayerEquipmentViewer extends ThemedGuiScreen {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                this.playerPreviewSlots.set(previewIndex,
                        copyOrEmpty(this.mc.player.getInventory().getNonEquipmentItems().get(inventoryIndex)));
            }
        }
        for (int col = 0; col < 9; col++) {
            this.playerPreviewSlots.set(27 + col, copyOrEmpty(this.mc.player.getInventory().getNonEquipmentItems().get(col)));
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE, this.guiLeft, this.guiTop, 0.0F, 0.0F,
                GUI_WIDTH, INVENTORY_ROWS * 18 + 17, 256, 256);
        graphics.blit(RenderPipelines.GUI_TEXTURED, CHEST_GUI_TEXTURE, this.guiLeft,
                this.guiTop + INVENTORY_ROWS * 18 + 17, 0.0F, 126.0F, GUI_WIDTH, 96, 256, 256);
