...
public class GuiItemNbtViewer extends ThemedGuiScreen {
    ...
    private void rebuildContent() {
        ...
        this.stack.save(savedTag);
        this.rawNbtText = savedTag.toString();

        String itemName = this.stack.isEmpty() ? "(空物品)" : this.stack.getHoverName().getString();
        ResourceLocation itemId = this.stack.isEmpty() ? null : BuiltInRegistries.ITEM.getKey(this.stack.getItem());
    ...
    private void notifyCopied() {
        if (this.mc != null && this.mc.player != null) {
            this.mc.player.sendSystemMessage(Component.literal("[物品查看器] 已复制原始NBT到剪贴板。")
