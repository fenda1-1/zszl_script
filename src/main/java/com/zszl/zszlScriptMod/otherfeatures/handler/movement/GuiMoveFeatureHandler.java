...
public final class GuiMoveFeatureHandler {
    ...
    public static void applyMovementInput(Minecraft mc, ClientInput movementInput) {
        ...
        float forwardImpulse = forwardDown == backDown ? 0.0F : (forwardDown ? 1.0F : -1.0F);
        float leftImpulse = leftDown == rightDown ? 0.0F : (leftDown ? 1.0F : -1.0F);
        if (sneakDown) {
            leftImpulse *= 0.3F;
            forwardImpulse *= 0.3F;
        }
        boolean sprintDown = isMovementKeyDown(mc.options.keySprint);
        movementInput.keyPresses = new Input(forwardDown, backDown, leftDown, rightDown, jumpDown, sneakDown, sprintDown);
        movementInput.moveVector = new Vec2(leftImpulse, forwardImpulse);

        LocalPlayer player = mc.player;
        if (player != null && isMovementKeyDown(mc.options.keySprint)
                && movementInput.moveVector.y > 0.0F
                && !movementInput.keyPresses.shift()) {
