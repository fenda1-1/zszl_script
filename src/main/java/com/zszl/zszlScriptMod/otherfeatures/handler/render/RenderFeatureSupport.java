...
final class RenderFeatureSupport {
    ...
    static void renderGroundSpeed(Minecraft mc, GuiGraphics graphics) {
        ...
        int panelWidth = Math.max(72,
                Math.max(RenderFeatureManager.groundSpeedShowTitle ? mc.font.width(title) : 0,
                        mc.font.width(value)) + 10);
        int lineCount = RenderFeatureManager.groundSpeedShowTitle ? 2 : 1;
        int panelHeight = mc.font.lineHeight * lineCount + 8 + (RenderFeatureManager.groundSpeedShowTitle ? 2 : 0);
        ...
        if (RenderFeatureManager.groundSpeedShowTitle) {
            ...
            graphics.drawString(mc.font, value, x + 5, y + 4, 0xFFFFFFFF, true);
        }
    }
