...
public final class UpdateManager {
    ...
    public static void fetchUpdateLinkAndOpen() {
        ...
        mc.player.displayClientMessage(new TextComponentString(ChatFormatting.AQUA + I18n.format("msg.update.fetching")), false);
        ...
        Thread thread = new Thread(() -> {
            try {
                    ...
                    try {
                        ...
                        if (mc.player != null) {
                            mc.player.displayClientMessage(new TextComponentString(
                                    ChatFormatting.AQUA + I18n.format("msg.update.password_copied", PASSWORD)), false);
                        }
                        if (openResult.opened()) {
                            if (mc.player != null) {
                                mc.player.displayClientMessage(new TextComponentString(
                                        ChatFormatting.AQUA + I18n.format("msg.update.opening_browser")), false);
                            }
                        } else if (mc.player != null) {
                            mc.player.displayClientMessage(new TextComponentString(
                                    ChatFormatting.RED + I18n.format("msg.update.browser_unsupported")), false);
                        }
                    } catch (Exception e) {
                        zszlScriptMod.LOGGER.error("Failed to open link or copy update URL on main thread", e);
                        if (mc.player != null) {
                            mc.player.displayClientMessage(new TextComponentString(
                                    ChatFormatting.RED + I18n.format("msg.update.operation_failed")), false);
                        }
                    }
                });
            } catch (Throwable t) {
                ...
                mc.execute(() -> {
                    if (mc.player != null) {
                        mc.player.displayClientMessage(new TextComponentString(
                                ChatFormatting.RED + I18n.format("msg.update.fetch_failed")), false);
                    }
                });
            }
        }, "UpdateManager-Fetch");
