...
public final class ModUtils {
    ...
    public static void setPlayerViewAngles(LocalPlayer player, float yaw, float pitch) {
        ...
        if (connection != null) {
            connection.send(new ServerboundMovePlayerPacket.Rot(normalizedYaw, normalizedPitch, player.onGround()));
        }
    }
    ...
    public static void sendJumpPacket(LocalPlayer player) {
        ...
        ClientPacketListener connection = Minecraft.getInstance().getConnection();
        if (connection != null) {
            connection.send(new ServerboundMovePlayerPacket.Pos(player.getX(), player.getY(), player.getZ(), false));
        }
    }
    ...
    public static void rightClickOnBlock(LocalPlayer player, BlockPos pos) {
        ...
        Direction direction = Direction.getNearest(diff.x, diff.y, diff.z).getOpposite();
        Vec3 hitVec = blockCenter.add(Vec3.atLowerCornerOf(direction.getNormal()).scale(0.5D));
    ...
    public static void simulateKey(String key, String state) {
        ...
        if (ModConfig.isDebugFlagEnabled(DebugModule.AHK_EXECUTION) && Minecraft.getInstance().player != null) {
            Minecraft.getInstance().player.sendSystemMessage(Component.literal("simulateKey " + key + " " + state));
        }
    }
    ...
    public static void simulateMouseClick(int x, int y, boolean isLeftClick, int originalWidth, int originalHeight,
        ...
        runOnClientThread(() -> {
            ...
            if (screen != null) {
                try {
                    screen.mouseClicked(scaledX, scaledY, button);
                    screen.mouseReleased(scaledX, scaledY, button);
                    return;
                } catch (Throwable t) {
    ...
    public static void useItemFromInventory(String itemName, int tempHotbarSlot, int switchDelayTicks,
        ...
        final int originalHotbarSlot = player.getInventory().selected;
    ...
    public static void sendFmlPacket(String channel, String hexData) {
        ...
        try {
            ...
            ResourceLocation identifier = ensureCustomPayloadIdentifier(resolveCustomPayloadIdentifier(channel), channel);
            FriendlyByteBuf payload = new FriendlyByteBuf(Unpooled.wrappedBuffer(data));
            listener.send(new ServerboundCustomPayloadPacket(identifier, payload));
        } catch (Exception e) {
            if (mc.player != null) {
                mc.player.sendSystemMessage(Component.literal("发送频道包失败: " + e.getMessage()));
            }
        }
    }
    ...
    public static void sendStandardPacketById(int packetId, String hexData) {
        ...
        try {
            String finalHex = resolvePacketHexPlaceholders(hexData);
            FriendlyByteBuf buffer = new FriendlyByteBuf(Unpooled.wrappedBuffer(parseHexToBytes(finalHex)));
            Packet<?> packet = ConnectionProtocol.PLAY.createPacket(PacketFlow.SERVERBOUND, packetId, buffer);
            if (packet == null) {
            ...
            if (mc.player != null) {
                mc.player.sendSystemMessage(Component.literal("发送标准包失败: " + e.getMessage()));
            }
        }
    }
    ...
    public static void mockReceiveFmlPacket(String channel, String hexData) {
        ...
        runOnClientThread(() -> {
            try {
                ...
                ResourceLocation identifier = ensureCustomPayloadIdentifier(resolveCustomPayloadIdentifier(channel), channel);
                FriendlyByteBuf payload = new FriendlyByteBuf(Unpooled.wrappedBuffer(data));
                ClientboundCustomPayloadPacket packet = new ClientboundCustomPayloadPacket(identifier, payload);
                PacketCaptureHandler.recordSyntheticPacket(false, packet.getClass().getSimpleName(), true, null,
                        channel, data);
                packet.handle(listener);
            } catch (Exception e) {
                if (mc.player != null) {
                    mc.player.sendSystemMessage(Component.literal("模拟接收频道包失败: " + e.getMessage()));
                }
            }
        });
    }
    ...
    public static void mockReceiveStandardPacketById(int packetId, String hexData) {
        ...
        runOnClientThread(() -> {
            try {
                ...
                if (mc.player != null) {
                    mc.player.sendSystemMessage(Component.literal("模拟接收标准包失败: " + e.getMessage()));
                }
            }
        });
    }
    ...
    private static int getSilentUseAutoRestoreDelayTicks(ItemStack stack) {
        ...
        if (useAnim == UseAnim.EAT || useAnim == UseAnim.DRINK) {
            return Math.min(40, Math.max(2, stack.getUseDuration() + 2));
        }
        if (useAnim == UseAnim.BOW || useAnim == UseAnim.BLOCK) {
            return 8;
        }
        return 1;
    }
    ...
    private static void syncMouseCursor(Minecraft mc, int rawX, int rawYTop) {
        ...
        try {
            GLFW.glfwSetCursorPos(mc.getWindow().getWindow(), rawX, rawYTop);
    ...
    static ResourceLocation resolveCustomPayloadIdentifier(String channel) {
        ...
        ResourceLocation legacy = ResourceLocation.tryParse(LEGACY_CHANNEL_NAMESPACE + ":" + sanitizeLegacyChannel(raw));
        return legacy != null ? legacy : defaultCustomPayloadIdentifier();
    }

    static ResourceLocation ensureCustomPayloadIdentifier(ResourceLocation identifier, String channelHint) {
        if (identifier != null) {
            return identifier;
        }
    ...
    private static ResourceLocation defaultCustomPayloadIdentifier() {
        return new ResourceLocation(LEGACY_CHANNEL_NAMESPACE, DEFAULT_CUSTOM_PAYLOAD_PATH);
    }
