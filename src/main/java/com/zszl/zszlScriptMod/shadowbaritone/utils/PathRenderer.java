...
public final class PathRenderer implements IRenderer {
    ...
    private static final Identifier TEXTURE_BEACON_BEAM = Identifier.withDefaultNamespace("textures/entity/beacon_beam.png");


    private PathRenderer() {}

    public static double posX() {
        return IRenderer.renderPosX();
    }

    public static double posY() {
        return IRenderer.renderPosY();
    }

    public static double posZ() {
        return IRenderer.renderPosZ();
    }
    ...
    private static void drawGoal(PoseStack stack, IPlayerContext ctx, Goal goal, float partialTicks, Color color, boolean setupRender) {
        ...
        if (goal instanceof IGoalRenderPos) {
            ...
            if (settings.renderGoalXZBeacon.value && drawGoalXZBeaconBeam(stack, ctx, goalPos, partialTicks, color, minY, maxY, renderPosX, renderPosY, renderPosZ)) {
                return;
            }
    ...
    private static boolean drawGoalXZBeaconBeam(PoseStack stack, IPlayerContext ctx, GoalXZ goalPos, float partialTicks,
                                                Color color, double minY, double maxY,
                                                double renderPosX, double renderPosY, double renderPosZ) {
        if (ctx == null || ctx.world() == null || ctx.minecraft() == null || ctx.minecraft().gameRenderer == null) {
            return false;
        }

        int beamStart = (int) minY;
        int beamHeight = Math.max(1, (int) (maxY - minY));
        float animationTime = settings.renderGoalAnimated.value
                ? (float) Math.floorMod(ctx.world().getGameTime(), 40L) + partialTicks
                : 0.0F;
        ...
        BeaconRenderer.submitBeaconBeam(
                stack,
                ctx.minecraft().gameRenderer.getSubmitNodeStorage(),
                TEXTURE_BEACON_BEAM,
                1.0F,
                animationTime,
                beamStart,
                beamHeight,
                color == null ? Color.GREEN.getRGB() : color.getRGB(),
