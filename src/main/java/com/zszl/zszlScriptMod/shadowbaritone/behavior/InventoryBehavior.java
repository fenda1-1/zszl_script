...
public final class InventoryBehavior extends Behavior implements Helper {
    ...
    public void onTick(TickEvent event) {
        ...
        int pick = bestPickaxeAgainst(Blocks.STONE);
        if (pick >= 9) {
    ...
    private int bestPickaxeAgainst(Block against) {
        NonNullList<ItemStack> invy = ctx.player().getInventory().getNonEquipmentItems();
        int bestInd = -1;
        double bestSpeed = -1;
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (ItemComponentCompat.isPickaxe(stack)) {
    ...
    public boolean throwaway(boolean select, Predicate<? super ItemStack> desired, boolean allowInventory) {
        ...
        if (desired.test(p.getInventory().getItem(40))) {
            ...
            for (int i = 0; i < 9; i++) {
                ItemStack item = inv.get(i);
                if (item.isEmpty() || ItemComponentCompat.isPickaxe(item)) {
                    if (select) {
                        p.getInventory().setSelectedSlot(i);
                    }
                    return true;
