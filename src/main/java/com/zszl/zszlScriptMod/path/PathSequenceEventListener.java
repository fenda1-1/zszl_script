...
public class PathSequenceEventListener {
    ...
    private boolean inventoryHasMatchingItem(LocalPlayer player, JsonObject params,
        ...
        List<ItemStack> mainInventory = player.getInventory().getNonEquipmentItems();
    ...
    private void captureInventorySlot(JsonObject params) {
        ...
        if ("HOTBAR".equalsIgnoreCase(area)) {
            if (slotIndex < mc.player.getInventory().getNonEquipmentItems().size()) {
                stack = mc.player.getInventory().getNonEquipmentItems().get(slotIndex);
            }
        } else if ("ARMOR".equalsIgnoreCase(area)) {
            if (slotIndex < 4) {
                stack = mc.player.getInventory().getItem(36 + slotIndex);
            }
        } else if ("OFFHAND".equalsIgnoreCase(area)) {
            if (slotIndex == 0) {
                stack = mc.player.getInventory().getItem(40);
            }
        } else if (slotIndex < mc.player.getInventory().getNonEquipmentItems().size()) {
            stack = mc.player.getInventory().getNonEquipmentItems().get(slotIndex);
        }
    ...
    private void captureHotbar(JsonObject params) {
        ...
        int filledCount = 0;
        for (int i = 0; i < 9 && i < mc.player.getInventory().getNonEquipmentItems().size(); i++) {
        ...
        int selectedIndex = mc.player.getInventory().getSelectedSlot();
        ...
        writeCapturedStack(varName + "_selected", mc.player.getInventory().getSelectedItem());
    }
    ...
    private String formatRegistryName(Identifier id) {
        if (id == null) {
            return "";
        }
    ...
    private String getStackNbtText(ItemStack stack) {
        return stack == null || stack.isEmpty() ? "" : stack.getComponents().toString();
    }
    ...
    private boolean runRetryExhaustedSequence(LocalPlayer player, PathSequenceManager.PathStep step) {
        ...
        if (!started && player != null) {
            player.displayClientMessage(new TextComponentString("§c[路径序列] 启动失败后执行序列失败: " + target), false);
        }
        return true;
    }
    ...
    private void sendPathRetryMessage(String message, ChatFormatting color) {
        ...
        player.displayClientMessage(new TextComponentString(color + "[路径序列] " + message), false);
    }
    ...
    private boolean isValidHuntCandidate(LocalPlayer player, LivingEntity entity) {
        ...
        return !huntEnableNameWhitelist
                || KillAuraHandler.getNameListMatchIndex(filterName, huntNameWhitelist) != Integer.MAX_VALUE;
    }
    ...
    private void drawHuntRangeAura(double centerX, double centerY, double centerZ, double radius,
        ...
        WorldGizmoRenderer.verticalRingWall(centerX, centerY, centerZ, radius,
                0.0D, 1.6D, new Color(0xFF5AC8), 0.72F, 2.0F, true);
    }
